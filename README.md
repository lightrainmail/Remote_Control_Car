...
#### 摇杆部分
首先摇杆要连接+3.3V电源和与单片机共地。

摇杆有X、Y两个方向，反正单片机资源够用，这里用了两个ADC，

摇杆Y方向引脚接PA0(ADC1_IN0)
![image](https://user-images.githubusercontent.com/89727667/222925482-99f9306f-607a-4056-ab4a-d6ba34bf8e98.png)

摇杆X方向引脚接PA1(ADC2_IN1)
![image](https://user-images.githubusercontent.com/89727667/222925506-00d2bf29-8bd7-424d-9830-973de4d92a48.png)

生产代码后，定义ADC1_GetValue();ADC2_GetValue();两个函数
...
uint16_t ADC1_GetValue(void){
    ...
    uint16_t ADC_Value;
    if(HAL_ADC_PollForConversion(&hadc1,10)==HAL_OK){//待ADC转换完成了，在读取ADC�?
        ADC_Value= HAL_ADC_GetValue(&hadc1);
    }
    return ADC_Value;
}

uint16_t ADC2_GetValu(void){
    HAL_ADC_Start(&hadc2);
    uint16_t ADC_Value;
    if(HAL_ADC_PollForConversion(&hadc2,10)==HAL_OK){//待ADC转换完成了，在读取ADC�?
...
定义后别忘声明，我在adc.c中定义，所以要在adc.h文件中声明
```c
...
#### 无线通信部份
这里的无线通信设置和小车部分的一摸一样，引脚都一样，用到的nrf24l01p.c和.h文件也都一样
